...
class Processor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        val map = resolver.getSymbolsWithAnnotation(Registry::class.qualifiedName!!).associateWith { it.validate() }

        map.filter { it.value }.keys
            .filterIsInstance<KSClassDeclaration>()
            .forEach { it.accept(RegistryVisitor(logger, codeGenerator), Unit) }
